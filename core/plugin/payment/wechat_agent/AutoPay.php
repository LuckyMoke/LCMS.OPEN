...
class AutoPay
{
    ...
    {
        global $_L;
        $order['payment']['notify_url']     = "{$_L['url']['site']}core/plugin/payment/wechat_agent/notify.php";
        $order['payment']['apiclient_cert'] = PATH_CORE_PLUGIN . "payment/wechat_agent/cert/" . md5($order['payment']['sub_mch_id']) . "_cert.pem";
        $order['payment']['apiclient_key']  = PATH_CORE_PLUGIN . "payment/wechat_agent/cert/" . md5($order['payment']['sub_mch_id']) . "_key.pem";
        return $order;
    }
    /**
     * [order 必要 下单接口]
    ...
    public static function order($order)
    {
        ...
        switch ($order['order']['paytype']) {
            case 'jsapi':
                load::plugin("payment/wechat_agent/type/WxPay.Jsapi");
                $WxPayJsapi = new WxPayJsapi();
                $result     = $WxPayJsapi->Order($config, $order['order']);
                require LCMS::template(PATH_CORE_PLUGIN . "payment/wechat_agent/tpl/jsapi");
                break;
            case 'h5':
                load::plugin("payment/wechat_agent/type/WxPay.H5");
                $WxPayH5 = new WxPayH5();
                ...
                break;
            case 'pc':
                load::plugin("payment/wechat_agent/type/WxPay.Pc");
                $WxPayPc = new WxPayPc();
                $result  = $WxPayPc->Order($config, $order['order']);
                if ($result['code_url']) {
                    $qrcode = urlencode($result['code_url']);
                    require LCMS::template(PATH_CORE_PLUGIN . "payment/wechat_agent/tpl/pc");
                } else {
                    LCMS::X($result['return_code'], $result['return_msg']);
                }
                break;
    ...
    public static function check($order)
    {
        ...
        load::plugin("payment/wechat_agent/Config");
        $config = new WxPayConfig($order['payment']);
        load::plugin("payment/wechat_agent/type/WxPay.Check");
    ...
    public static function repay($order)
    {
        ...
        load::plugin("payment/wechat_agent/Config");
        $config = new WxPayConfig($order['payment']);
        load::plugin("payment/wechat_agent/type/WxPay.Repay");
    ...
    {
        load::plugin("payment/wechat_agent/type/WxPay.Notify");
    ...
    public static function payto($order)
    {
        ...
        load::plugin("payment/wechat_agent/Config");
        $config = new WxPayConfig($order['payment']);
        load::plugin("payment/wechat_agent/type/WxPay.To");
        $WxPayTo = new WxPayTo();
        $result  = $WxPayTo->Order($config, $order['order']);
        if ($result['result_code'] == "SUCCESS") {
            return ["code" => 1, "msg" => "发送成功", "order_no" => $result['mch_billno']];
        } else {
